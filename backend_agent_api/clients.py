...
def get_graph_client():
    """
    Get or create GraphitiClient for knowledge graph operations.
    Returns None if graph is not configured or available.
    """
    try:
        from graph_utils import create_graph_client

        # Attempt to create graph client
        graph_client = create_graph_client()

        if graph_client:
            print("Graph client initialized successfully")
        else:
            print("Graph client not available - knowledge graph features disabled")

        return graph_client

    except ImportError as e:
        print(f"Graph utilities not available: {e}")
        return None
    except Exception as e:
        print(f"Failed to initialize graph client: {e}")
        return None

async def initialize_graph_client(graph_client):
    """
    ...
    if graph_client:
        try:
            ...
            print("Graph client initialized and connected")
            return graph_client
        except Exception as e:
            print(f"Failed to initialize graph client: {e}")
            return None
