...
"""
        ...
        if graph_client:
            try:
                graph_search_results = await graph_client.search(user_query)
                if graph_search_results:
                    graph_facts = []
                    for fact in graph_search_results[:3]:  # Limit to top 3 graph facts
                        graph_facts.append(f"• {fact.get('fact', 'Unknown fact')}")

                    if graph_facts:
                        graph_results = "\n\n## Knowledge Graph Context:\n" + "\n".join(graph_facts)
            except Exception as e:
                print(f"Graph search failed in retrieve_relevant_documents: {e}")
                # Continue with just vector results if graph fails
        ...
        return combined_results if combined_results else "No relevant documents found."
...
async def graph_search_tool(graph_client, query: str, search_type: str = "hybrid") -> str:
    """
    Search knowledge graph for entities and relationships.

    Args:
        graph_client: The GraphitiClient instance
        query: Search query
    ...
    print(graph_client)
    if not graph_client:
        print("Graph search unavailable - using vector search only")
    ...
    try:
        ...
        if not results:
            print("No graph results found for the query.")
            return "No graph results found for the query."
        ...
        for result in results[:10]:  # Limit to top 10 results
            formatted_results.append(
                f"• {result.get('fact', 'Unknown fact')}\n"
                f"  (Valid at: {result.get('valid_at', 'Unknown time')})"
            )
...
async def entity_relationships_tool(graph_client, entity_name: str, depth: int = 2) -> str:
    """
    ...
    Args:
        ...
        depth: Maximum depth to traverse (not fully used by Graphiti)

    Returns:
        Formatted entity relationships or error message
    """
    if not graph_client:
        return "Entity relationship search unavailable - graph not configured"

    try:
        # Get related entities from graph
        relationships = await graph_client.get_related_entities(entity_name, depth=depth)

        if not relationships or not relationships.get('related_facts'):
            return f"No relationships found for entity: {entity_name}"

        # Format the results
        facts = relationships.get('related_facts', [])
        formatted_results = [f"Entity: {entity_name}\n\nRelated Facts:"]

        for i, fact in enumerate(facts[:15], 1):  # Limit to top 15 facts
            formatted_results.append(
        ...
        print(f"Entity relationship search failed: {e}")
        return f"Entity relationship search error: {str(e)}"

async def entity_timeline_tool(graph_client, entity_name: str) -> str:
    """
    Get temporal timeline of facts for an entity from the knowledge graph.
    ...
    Returns:
        Formatted timeline or error message
    """
    if not graph_client:
        return "Entity timeline unavailable - graph not configured"

    try:
        # Get timeline from graph
        timeline = await graph_client.get_entity_timeline(entity_name)

        if not timeline:
            return f"No timeline found for entity: {entity_name}"

        # Format the timeline
        formatted_results = [f"Timeline for {entity_name}:\n"]

        for event in timeline[:20]:  # Limit to top 20 events
            valid_at = event.get('valid_at', 'Unknown time')
            fact = event.get('fact', 'Unknown event')
            formatted_results.append(f"• [{valid_at}] {fact}")

        return "\n".join(formatted_results)
    except Exception as e:
        print(f"Entity timeline search failed: {e}")
        return f"Entity timeline error: {str(e)}"
