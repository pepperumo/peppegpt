# PeppeGPT - Knowledge Graph RAG Workshop

> **🚀 Workshop Branch**: This branch showcases the **Knowledge Graph-Enhanced RAG** implementation used for PeppeGPT - integrating Graphiti and Neo4j for knowledge graph RAG to enhance semantic understanding, entity relationships, and temporal reasoning for the agent.

## Module 6: AI Agent Deployment

A modular, production-ready deployment setup for PeppeGPT. This module restructures the code from previous modules into independently deployable components: the AI agent API, RAG pipeline, and frontend application. Each component can be deployed, scaled, and maintained separately (or together!) while working together as a cohesive system.
...
- Optional: Neo4j database for knowledge graph features (can be local, cloud, or Docker)
...
This branch includes knowledge graph capabilities powered by Neo4j and Graphiti, enhancing RAG with entity relationships, temporal reasoning, and hybrid search. The system works seamlessly without Neo4j - it will automatically fall back to vector-only search if the graph database is unavailable.

### Quick Setup

1. **Deploy Neo4j** (choose one):
   ```bash
   ...
   # Option 2: Neo4j Desktop or AuraDB (cloud)
   # Download from https://neo4j.com/download/
   ```

2. **Configure in `.env`**:
   ```env
   # Neo4j Knowledge Graph (optional - leave empty for vector-only mode)
...
3. **What It Provides**:
   - **Entity Extraction**: Automatically identifies companies, people, technologies, and locations
   - **Relationship Mapping**: Tracks connections between entities with temporal awareness
   - **Hybrid Search**: Combines vector similarity with graph traversal for superior results
   - **Agent Tools**: New capabilities for entity timeline queries and relationship exploration

The knowledge graph uses your existing LLM and embedding configurations - no additional API keys needed.
