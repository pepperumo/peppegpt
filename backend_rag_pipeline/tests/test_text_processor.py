...
class TestExtractTextFromPdf:
    ...
    def test_extract_text(self, mock_docling_extract):
        """Test extracting text from PDF using Docling"""
        # Setup mock
        mock_docling_extract.return_value = "# PDF Document\n\nExtracted text from PDF."
        ...
        mock_docling_extract.assert_called_once_with(
            b'fake pdf content',
            'document.pdf',
            'application/pdf'
        )
...
class TestExtractTextFromFile:
    @patch('common.text_processor.extract_text_with_docling')
    def test_pdf_file(self, mock_docling_extract):
        """Test extracting text from PDF file"""
        mock_docling_extract.return_value = "PDF content"
    ...
    def test_with_text(self, openai_client_mock):
        ...
        with patch('common.text_processor.openai_client', openai_client_mock):
            ...
            openai_client_mock.embeddings.create.assert_called_once_with(
                model="text-embedding-3-small",  # Default from os.getenv("EMBEDDING_MODEL_CHOICE", "text-embedding-3-small")
                input=["Text 1", "Text 2"]
            )
