...
- **Versatile File Processing:** Processes various document types including PDFs, text documents, HTML, CSVs, DOCX, PPTX, and more (see Supported File Types).
- **Automated Text Extraction & Chunking:** Extracts text content with Docling (local processing) and intelligently splits it into manageable chunks (default: 400 characters, configurable).
- **Advanced OCR with Docling:** Local document processing with OCR support for scanned PDFs and images - no external API costs.
- **Table Extraction from PDFs:** Automatically extracts tables from PDFs as structured data for SQL queries.
...
The pipeline uses Docling for local document processing with OCR and table extraction capabilities. You can configure Docling's behavior through environment variables:

### Environment Variables

-   `DOCLING_OCR_LANGUAGES` (default: `en`): Comma-separated list of language codes for OCR processing.
    -   Supported languages: `en`, `de`, `fr`, `es`, `it`, `pt`, `nl`, `ru`, `ar`, `zh`, `ja`, `ko`, and more
    -   Example: `DOCLING_OCR_LANGUAGES=en,de,fr` for English, German, and French
    
-   `DOCLING_TABLE_MODE` (default: `accurate`): Table extraction quality mode.
    -   `fast`: Quicker processing, less accurate for complex tables
    -   `accurate`: Better quality extraction for complex tables (recommended)
    
-   `DOCLING_DEVICE` (default: `auto`): Hardware acceleration device.
    -   `auto`: Automatically detects and uses GPU/MPS if available, falls back to CPU
    -   `cpu`: Force CPU-only processing
    -   `cuda`: Use NVIDIA GPU (Linux only)
    -   `mps`: Use Apple Silicon GPU acceleration (macOS only)
    
-   `DOCLING_NUM_THREADS` (default: `4`): Number of threads for processing.
    -   Increase for better performance on multi-core systems
    -   Example: `DOCLING_NUM_THREADS=8`

### Hardware Acceleration

Docling automatically detects and uses available hardware acceleration:

- **Apple Silicon (M1/M2/M3)**: Automatically uses Metal Performance Shaders (MPS) for GPU acceleration
- **NVIDIA GPUs**: Automatically uses CUDA for faster processing (Linux)
- **CPU fallback**: Works on any system without GPU

**Performance Note**: GPU acceleration can significantly improve processing speed for large documents and batch processing, especially for OCR and table extraction operations.

### Docker Configuration

The Docker image includes Tesseract OCR dependencies for optimal Docling performance:
- `tesseract-ocr`: Core OCR engine
- `tesseract-ocr-eng`: English language pack (add more languages as needed)
...
3.  **File Processing (for new/updated files):**
    ...
    -   Text content is extracted using Docling for local, efficient processing of PDFs, DOCX, PPTX, and images with OCR support.
    -   For Google Workspace files, they are typically exported to a plain text or CSV format first, as defined in `Google_Drive/config.json`.
    -   Tables in PDFs are automatically extracted as structured data and stored for SQL queries.
...
-   PDF (`application/pdf`) - with advanced table extraction and OCR support
-   Plain Text (`text/plain`)
-   HTML (`text/html`)
-   CSV (`text/csv`)
-   Microsoft Excel (`application/vnd.ms-excel`, `application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`)
-   Microsoft Word (`application/vnd.openxmlformats-officedocument.wordprocessingml.document`) - processed with Docling
-   Microsoft PowerPoint (`application/vnd.openxmlformats-officedocument.presentationml.presentation`) - processed with Docling
-   Google Docs, Sheets, Presentations (for Google Drive, converted as per `export_mime_types`)
-   Images (`image/png`, `image/jpg`, `image/jpeg`, `image/svg`) - OCR processed with Docling locally
    *Note: All document processing happens locally with Docling - no external API dependencies or costs for OCR.*
