...
from common.db_handler import process_file_for_rag, delete_document_by_file_id, supabase
...
class GoogleDriveWatcher:
    ...
    def download_file(self, file_id: str, mime_type: str) -> Optional[bytes]:
        ...
        if not self.service:
            print(f"Error: Google Drive service not authenticated")
            return None
    ...
    def check_for_deleted_files(self) -> List[str]:
        ...
        This checks ALL files in the database, not just known_files.
        ...
        try:
            ...
            if not result.data:
                print("No files in database to check for deletion")
                return deleted_files
            
            print(f"Checking {len(result.data)} files from database against Google Drive...")
            ...
            for doc in result.data:
                ...
                try:
                    ...
                    if file.get('trashed', False):
                        print(f"File '{file_name}' (ID: {file_id}) is in trash")
                        deleted_files.append(file_id)
                except Exception as e:
                    # If we get an error (like file not found), the file is deleted
                    if 'File not found' in str(e) or '404' in str(e):
                        print(f"File '{file_name}' (ID: {file_id}) not found in Google Drive - marking for deletion")
                        deleted_files.append(file_id)
                    else:
                        print(f"Error checking file '{file_name}' ({file_id}): {e}")
            
            if deleted_files:
                print(f"Found {len(deleted_files)} files to delete from database")
        except Exception as e:
            print(f"Error in check_for_deleted_files: {e}")
    ...
    def check_for_changes(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if not self.initialized:
                ...
                # Skip the regular change check on first run since we just did initialization
                changed_files = []
            else:
